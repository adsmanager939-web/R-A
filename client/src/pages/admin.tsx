    ...
    <div className="min-h-screen bg-[#0f2438] flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow-2xl p-10 w-full max-w-sm">
        <div className="flex flex-col items-center mb-8">
          <div className="bg-[#1a3a52] rounded-full p-4 mb-4">
            <ShieldAlert className="w-8 h-8 text-[#d4af37]" />
          </div>
          <h1 className="text-2xl font-bold text-[#1a3a52]">Admin Access</h1>
          <p className="text-gray-500 text-sm mt-1">TrueClaim & Associates</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button
            type="submit"
            className="w-full bg-[#1a3a52] hover:bg-[#2a4a62] text-white"
        ...
        <div className="mt-6 text-center">
          <Link href="/" className="text-sm text-[#1a3a52] hover:text-[#d4af37] transition-colors">
            ← Back to website
          </Link>
        </div>
      </div>
...
function Dashboard({ password, onLogout }: { password: string; onLogout: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-gradient-to-r from-[#1a3a52] to-[#2a4a62] text-white px-6 py-4 flex items-center justify-between shadow-lg">
        <div className="flex items-center gap-3">
          <ShieldAlert className="w-6 h-6 text-[#d4af37]" />
          <div>
            <h1 className="font-bold text-lg leading-tight">TrueClaim Admin</h1>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex items-center gap-3 mb-6">
          <div className="bg-[#1a3a52] text-white rounded-xl px-4 py-3 flex items-center gap-2">
            ...
            {sorted.map((s) => (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                  <div>
                    <p className="font-semibold text-[#1a3a52] text-lg" data-testid={`text-name-${s.id}`}>{s.name}</p>
                    <p className="text-gray-500 text-sm">{new Date(s.createdAt).toLocaleString("en-CA", { dateStyle: "medium", timeStyle: "short" })}</p>
                  </div>
                  <div className="flex gap-2 flex-wrap">
                    <Badge className="bg-[#1a3a52] text-white hover:bg-[#1a3a52]">
                      {SCAM_LABELS[s.scamType] ?? s.scamType}
                    </Badge>
                    {s.amountLost && (
                      <Badge variant="outline" className="border-[#d4af37] text-[#b8941f]">
                        Lost: {s.amountLost}
                      </Badge>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm text-gray-600 mb-3">
                  <p><span className="font-medium text-gray-700">Email:</span> <a href={`mailto:${s.email}`} className="text-[#1a3a52] hover:underline" data-testid={`text-email-${s.id}`}>{s.email}</a></p>
                  <p><span className="font-medium text-gray-700">Phone:</span> <a href={`tel:${s.phone}`} className="text-[#1a3a52] hover:underline" data-testid={`text-phone-${s.id}`}>{s.phone}</a></p>
                </div>
                {s.description && (
                  <div className="bg-gray-50 rounded-lg p-3 text-sm text-gray-600 border-l-4 border-[#d4af37]">
